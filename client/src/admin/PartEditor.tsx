...
import { STATES, VEHICLES } from '../lib/types';
import { resizeImage } from '../lib/image';
import { isUploadedImage, uploadImage } from '../lib/catalogue';
...
export function PartEditor({
  ...
  async function pickFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onChange({ image: await uploadImage(blob) });
    } catch (err) {
      setImageError(err instanceof Error ? err.message.toUpperCase() : "ENVOI DE L'IMAGE IMPOSSIBLE");
    } finally {
      setImageBusy(false);
    }
  }
